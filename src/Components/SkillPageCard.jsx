        ...
        <div className={`animate-in slide-in-from-${id % 2 == 0 ? 'right' : 'left'}-96 duration-500`}>
            ...
            <div className="grid gap-8 md:gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-6 mt-3 mb-4">
                {skill.codes.filter(code => {
                    ...
                    .map((code, id) => (
                        <div
                            className="flex flex-col border cursor-pointer hover:scale-105 hover:opacity-80 transition-all ease-in duration-150 border-zinc-200 dark:border-2 dark:border-zinc-500 shadow-lg dark:shadow-zinc-600 bg-indigo-50 dark:bg-zinc-900/40 font-semibold items-center rounded-2xl"
                            key={id}
